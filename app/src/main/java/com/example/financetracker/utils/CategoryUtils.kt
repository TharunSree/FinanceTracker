...
object CategoryUtils {
    ...
    fun getDefaultCategories(): List<Category> {
        return defaultCategoryNames.map { name ->
            Category(
                id = 0, // ID will be generated by Room if inserted
                name = name,
                userId = null, // Default categories might not be tied to a specific user initially
                isDefault = true, // Mark these as system defaults
                colorHex = null // Default categories might not have a color initially
            )
        }
    }
    ...
    suspend fun addCategory(context: Context, category: Category) {
        // Ensure category has a user ID
        val userId = category.userId ?: run {
            Log.e(TAG, "Cannot add category without userId: $category")
            throw IllegalArgumentException("Category must have a userId") // Throw error
        }
        // Ensure it's not marked as default (user categories shouldn't be)
        val categoryToAdd = category.copy(isDefault = false, id = 0) // Ensure ID is 0 for insert
        ...
        withContext(Dispatchers.IO) {
            try {
                // Check if category already exists locally for this user
                val existing = database.categoryDao().getCategoryByName(categoryToAdd.name, userId)
                if (existing == null) {
                    // Save to local database
                    val newId = database.categoryDao().insertCategory(categoryToAdd) // Get new ID
                    Log.i(TAG, "Added category '${categoryToAdd.name}' to Room for user $userId with ID $newId.")

                    // Save to Firestore if not guest user
                    if (!GuestUserManager.isGuestMode(userId) && FirebaseAuth.getInstance().currentUser != null) {
                        // Check Firestore existence before adding? Optional, depends on desired behaviour.
                        firestore.collection("users")
                            .document(userId)
                            .collection("categories")
                            .add(mapOf(
                                "name" to categoryToAdd.name,
                                "isDefault" to categoryToAdd.isDefault, // Should be false
                                "colorHex" to categoryToAdd.colorHex // Add color
                            ))
                    ...
                    throw IllegalStateException("Category '${categoryToAdd.name}' already exists.") // Throw error
                }
            } catch (e: Exception) {
    ...
    suspend fun updateCategory(context: Context, category: Category) {
        ...
        val categoryToUpdate = category.copy(isDefault = false)
        ...
        withContext(Dispatchers.IO) {
            try {
                // Fetch the original category from DB to check if it was default
                val originalCategory = database.categoryDao().getCategoryById(categoryToUpdate.id, userId)
                if (originalCategory == null) {
                    Log.e(TAG, "Cannot update category: Original category not found with ID ${categoryToUpdate.id} for user $userId")
                ...
                database.categoryDao().updateCategory(categoryToUpdate)
                Log.i(TAG, "Updated category '${categoryToUpdate.name}' in Room for user $userId.")

                // Update Firestore if not guest user
                if (!GuestUserManager.isGuestMode(userId) && FirebaseAuth.getInstance().currentUser != null) {
                    // Find the corresponding document in Firestore to update
                    // Querying by name is problematic if name changes. Need Firestore doc ID or rely on name match.
                    // Assuming name is the identifier for now, but using ID stored elsewhere would be better.
                    firestore.collection("users")
                        .document(userId)
                        .collection("categories")
                        ...
                        .forEach { doc ->
                            doc.reference.update(mapOf(
                                "name" to categoryToUpdate.name, // Update name
                                "isDefault" to categoryToUpdate.isDefault, // Should be false
                                "colorHex" to categoryToUpdate.colorHex // Update color
                            ))
                                .addOnSuccessListener { Log.d(TAG, "Updated category '${categoryToUpdate.name}' in Firestore.") }
                                .addOnFailureListener { e -> Log.e(TAG, "Failed to update category '${categoryToUpdate.name}' in Firestore.", e) }
                        }
                }
            } catch (e: Exception) {
                Log.e(TAG, "Error updating category '${categoryToUpdate.name}' for user $userId", e)
                throw e // Re-throw
            }
        }
    }
    ...
    suspend fun deleteCategory(context: Context, category: Category) {
        ...
        withContext(Dispatchers.IO) {
            try {
                // Fetch the category again to double-check the isDefault flag from DB
                // **** CORRECTED CALL TO getCategoryById ****
                val dbCategory = database.categoryDao().getCategoryById(category.id, userId) // Pass both id and userId
                ...
                database.categoryDao().deleteCategory(category) // Pass the original object
                Log.i(TAG, "Deleted category '${category.name}' from Room for user $userId.")

                // Delete from Firestore if not guest user
                if (!GuestUserManager.isGuestMode(userId) && FirebaseAuth.getInstance().currentUser != null) {
                    // Find and delete the corresponding document in Firestore
                    firestore.collection("users")
                        .document(userId)
                        .collection("categories")
                        .whereEqualTo("name", category.name) // Query by name
                        .get()
                        .await()
                        .documents
                        .forEach { doc ->
                            doc.reference.delete()
                                .addOnSuccessListener { Log.d(TAG, "Deleted category '${category.name}' from Firestore.") }
                                .addOnFailureListener { e -> Log.e(TAG, "Failed to delete category '${category.name}' from Firestore.", e) }
                        }
                }
            } catch (e: Exception) {
